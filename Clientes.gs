function sincronizarYLimpiarCRM() {
  ...
  // Estructura: [0]ID, [1]Cliente, [2]Riv, [3]PS, [4]FP, [5]Dir, [6]Tel, [7]Email, [8]CUIL
  ...
  for (let j = 1; j < datosCRM.length; j++) {
    // Buscamos el nombre en las columnas donde pudo haber quedado (A, B o C)
    ...
    if (nombreReal) {
      ...
      clientesProcesados[nombreReal] = [
        ...
        ref[6], // E: Telefono
        ref[7], // F: Email
        ref[2] || datosCRM[j][6] || "", // G: Rivadavia (Prioriza origen, luego CRM)
        ref[3] || datosCRM[j][7] || "", // H: Provincia (Prioriza origen, luego CRM)
        ref[4] || datosCRM[j][8] || ""  // I: FedPatronal (Prioriza origen, luego CRM)
      ];
    }
  }
  ...
  for (let nombreRef in mapaReferencia) {
    if (!clientesProcesados[nombreRef]) {
      ...
      clientesProcesados[nombreRef] = [
        d[0], d[1], d[8], d[5], d[6], d[7], 
        d[2], // G: Rivadavia de Agrupados
        d[3], // H: Provincia de Agrupados
        d[4]  // I: FedPatronal de Agrupados
      ];
    }
  }
